...
const TableH = ({list, deleteId}) => {

  const {estacion, linea, fecha, funcionario, lorry, sonda1, sonda2, presion, horario, id} = list;
    ...
    <>
      <tr className='border-b'>
        ...
        <td className='text-center flex gap-2 justify-center flex-row items-center py-6'>
          <Link to={`/updateH/${id}`}>
            <button onClick={() => setIdH(list)} className='text-green-500 hover:text-green-700 text-1xl font-semibold'>Editar</button>
          </Link>
